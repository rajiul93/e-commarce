      ...
      <div className="flex text-amber-400">
        {Array.from({ length: 5 }).map((_, i) => (
          <svg
            key={i}
            viewBox="0 0 20 20"
            className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${i < full || (i === full && half) ? 'fill-current' : 'fill-zinc-200 text-zinc-200'}`}
          >
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 0 0 .95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 0 0-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 0 0-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 0 0-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 0 0 .951-.69l1.07-3.292z" />
          </svg>
        ))}
      </div>
      <span className="text-[10px] font-medium text-zinc-600 sm:text-xs">{value.toFixed(1)}</span>
    </div>
  );
}
  ...
  return (
    <article className="group/card relative flex h-full flex-col overflow-hidden  bg-white shadow-sm ring-1 ring-zinc-950/5 transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-zinc-900/10">
        ...
        <div className="relative aspect-[4/5] overflow-hidden">
          ...
          {!inStock ? (
            <div className="absolute inset-0 flex items-center justify-center bg-zinc-900/10 backdrop-blur-[1px]">
              <span className="rounded-full bg-zinc-900/80 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white sm:px-3 sm:py-1 sm:text-xs">
                Sold out
              </span>
            </div>
          ) : null}
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3 sm:gap-2 sm:p-4">
        <Link href={`/products/${product.slug}`} className="min-w-0 space-y-0.5 sm:space-y-1">
          {product.brand?.brandName ? (
            <p className="truncate text-[10px] font-semibold uppercase tracking-widest text-zinc-400 sm:text-[11px]">
              {product.brand.brandName}
            </p>
          ) : product.category ? (
            <p className="truncate text-[10px] font-semibold uppercase tracking-widest text-zinc-400 sm:text-[11px]">
          ...
          <h3
            className="truncate text-xs font-semibold text-zinc-900 transition group-hover/card:text-primary sm:text-sm"
            title={product.title}
          >
            {product.title}
          </h3>
        ...
        <div className="mt-auto space-y-1.5 pt-0.5 sm:space-y-2 sm:pt-1">
          {onOffer && product.minPrice != null ? (
            <div className="flex flex-wrap items-end gap-1.5 sm:gap-2">
              ...
              <p className="pb-0.5 text-[10px] text-zinc-400 line-through sm:text-xs">
                {formatPriceRange(product.minPrice, product.maxPrice)}
              </p>
              {offerLabel ? (
                <span className="rounded-md bg-primary/10 px-1 py-0.5 text-[9px] font-bold text-primary sm:px-1.5 sm:text-[10px]">
                  {offerLabel}
                </span>
              ) : null}
            </div>
          ...
          <div className="flex items-center justify-between gap-2">
            {lowStock ? (
              <p className="text-[10px] font-medium text-amber-600 sm:text-[11px]">
                Only {totalStock} left
              </p>
            ) : inStock ? (
              <p className="text-[10px] font-medium text-emerald-600 sm:text-[11px]">In stock</p>
            ) : null}
          ...
          <ProductCardAddToCart
            ...
            slug={product.slug}
            inStock={inStock}
            className="!py-2 !text-[10px] sm:!py-2.5 sm:!text-xs"
          />
        </div>
      </div>
    </article>
